...
def _room(room_text):
    ...
    if "studio" in t or "bed flat" in t or "bed house" in t:
        cat = "whole property"
...
def scrape(slug, ad_type, max_pages):
    ...
    for page in range(1, max_pages + 1):
        url = f"{BASE}/{AD_TYPES[ad_type]}/{slug}" + (f"/page{page}" if page > 1 else "")
        body = fetch(url)
        if body is None:
            break
        pages += 1
        ...
        if not new:  # only repeated featured ads left -> end of results
            break
...
values (%(listing_id)s, %(area)s, %(postcode_district)s, %(neighbourhood)s, %(property_type)s,
  ...
  %(verified)s, %(days_old)s, %(today)s, %(today)s)
...
insert into market.wanted_listings (listing_id, search_area, budget_pcm, headline_rate, headline_period,
  ...
  days_old_at_first_seen, first_seen, last_seen)
values (%(listing_id)s, %(area)s, %(budget_pcm)s, %(headline_rate)s, %(headline_period)s,
  ...
  %(early_bird)s, %(brand)s, %(days_old)s, %(today)s, %(today)s)
...
def save(conn, area, ad_type, rows, pages, today, run_id):
    ...
    with conn.transaction(), conn.cursor() as cur:
        ...
        cur.execute("""update market.scrape_runs set status='success', finished_at=now(),
                       pages=%s, listings=%s where run_id=%s""", (pages, len(rows), run_id))
...
def main():
    ...
    for area, slug in areas.items():
        for ad_type in (args.ad_type or AD_TYPES):
            ...
            try:
                rows, pages = scrape(slug, ad_type, args.max_pages)
                if not rows:
                    raise RuntimeError("no listings parsed - page layout may have changed")
                if conn:
                    save(conn, area, ad_type, rows, pages, today, run_id)
                else:
                    print(json.dumps({"area": area, "ad_type": ad_type, "pages": pages,
                                      "listings": len(rows), "sample": rows[:2]}, indent=1, default=str))
                print(f"{area} {ad_type}: {len(rows)} listings from {pages} pages", file=sys.stderr)
